...
class TestLesson:
    ...
    def test_search_on_google(self, page):
        ...
        if base_page.is_visible('button:has-text("Accept all")'):
            base_page.click_to_element('button:has-text("Accept all")')

        base_page.fill_text('//textarea[@id="APjFqb"]', "Playwright Python")
        base_page.click_to_element("(//span[text() = 'playwright python'])[1]")
